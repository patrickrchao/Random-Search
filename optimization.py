...
class optimizer:
    def __init__(self,optimization_params,step_params,optimizer_type="NORMALIZE"):
        self.iterations = optimization_params["ITERATIONS"]
        self.nu = optimization_params["NU"]
        self.queries = optimization_params["QUERIES"]//2 +1
        self.optimizer_type = optimizer_type

        self.step_function = optimization_step(step_params,self.optimizer_type)
    ...
    def gradient_descent(self,oracle,initialization):
        ...
        for curr_iter in range(1,self.iterations):
            ...
            x_values[:,curr_iter] = x_values[:,curr_iter-1]-update.T*alpha/(self.nu*curr_sigma*self.queries)
    ...
    def adagrad_optimize(self,oracle,initialization):
        ...
        for curr_iter in range(1,self.iterations):
            function_evals = np.zeros((self.queries,2))
            for i in range(self.queries):
                curr_grad= np.array([oracle.derivative(curr_x) for curr_x in x_values[:,curr_iter-1]])
